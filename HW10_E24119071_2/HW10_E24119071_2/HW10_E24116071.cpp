...
// of substring `X[0¡Km-1]` and `Y[0¡Kn-1]`
...
set<string> LCS(string X, string Y)
{
	...
	// lookup[i][j] stores the length of LCS of substring `X[0¡Ki-1]` and `Y[0¡Kj-1]`
...
int main()
{
	...
	set<string> allAnswer = LCS(s1, s2);

	for (string str : allAnswer) {
		cout << str << endl;
	}
